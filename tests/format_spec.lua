...
describe("format.render", function()
  ...
  it("folders render with a trailing slash in bare format", function()
    local insert, label = format.render("bare", "foo/bar", true)
    assert.equals("@foo/bar/ ", insert)
    assert.equals("foo/bar/", label)
  end)

  it("folders render with a trailing slash in markdown format", function()
    local insert, label = format.render("markdown", "foo/bar", true)
    assert.equals("[@bar/](foo/bar/) ", insert)
    assert.equals("foo/bar/", label)
  end)
end)
